...
- (void)setupAllCityList {
    NSString *path = [[NSBundle mainBundle] pathForResource:@"allCity" ofType:@"txt"];
    ...
    if (data) {
        tmpArr = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
//        _cityArray = tmpArr[0][@"sub"];
        for (NSDictionary *dic in tmpArr) {
            [citys addObject:dic[@"name"]];
            
            [cityDic setObject:dic[@"citycode"] forKey:dic[@"name"]];
        }
    }
    self.mutableDic = [cityDic mutableCopy];
    _cityMutableArray = [citys mutableCopy];
}
    ...
    if ([[cityDic valueForKey:@"cityName"] isEqualToString:@"全城"]) {
        ...
        [_manager currentCity:[KCURRENTCITYINFODEFAULTS objectForKey:@"cityNumber"] currentCityName:^(NSString *name) {
            ...
            if (weakSelf.choseCityBlock) {
                weakSelf.choseCityBlock(name, [self.mutableDic objectForKey:name]);
        ...
        if (self.choseCityBlock) {
            self.choseCityBlock(cityName, [self.mutableDic objectForKey:cityName]);
...
- (JFSearchView *)searchView {
    if (!_searchView) {
        ...
        [_searchView resultBlock:^(NSDictionary *cityData) {
            ...
            if (weakSelf.choseCityBlock) {
                weakSelf.choseCityBlock([cityData valueForKey:@"city"],[self.mutableDic objectForKey:[cityData valueForKey:@"city"]]);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.choseCityBlock) {
        ...
        self.choseCityBlock(cell.textLabel.text, [self.mutableDic objectForKey:cell.textLabel.text]);
    }
